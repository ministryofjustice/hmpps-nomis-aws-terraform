...
#-------------------------------------------------------------
data "terraform_remote_state" "nomis_security_groups" {
  backend = "s3"
...
data "terraform_remote_state" "key_profile" {
  ...
  config {
    bucket = "${var.remote_state_bucket_name}"
    key    = "key_profile/terraform.tfstate"
    region = "${var.region}"
  }
}
...
data "terraform_remote_state" "database_failover" {
  ...
  config {
    bucket = "${var.remote_state_bucket_name}"
    key    = "database_failover/terraform.tfstate"
    region = "${var.region}"
  }
}

#-------------------------------------------------------------
### Getting the nomis buckets
#-------------------------------------------------------------
data "terraform_remote_state" "s3buckets" {
  ...
  config {
    bucket = "${var.remote_state_bucket_name}"
    key    = "s3buckets/terraform.tfstate"
    region = "${var.region}"
  }
}
