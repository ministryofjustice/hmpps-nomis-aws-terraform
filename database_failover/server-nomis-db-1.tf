module "nomis_db_1" {
  ...
  security_group_ids = [
    ...
    "${data.terraform_remote_state.nomis_security_groups.sg_nomis_db_in_id}",
    "${data.terraform_remote_state.nomis_security_groups.sg_nomis_db_out_id}",
  ...
  ansible_vars = {
    ...
    ## oradb_sys_password            = "/${environment_name}/${project}/nomis-database/db/oradb_sys_password"
    ## oradb_system_password         = "/${environment_name}/${project}/nomis-database/db/oradb_system_password"
    ## oradb_sysman_password         = "/${environment_name}/${project}/nomis-database/db/oradb_sysman_password"
    ## oradb_dbsnmp_password         = "/${environment_name}/${project}/nomis-database/db/oradb_dbsnmp_password"
    ## oradb_asmsnmp_password        = "/${environment_name}/${project}/nomis-database/db/oradb_asmsnmp_password"
  }
}

#legacy (used for info only)
output "ami_nomis_db_1" {
  value = "${module.nomis_db_1.ami_id}"
}

output "public_fqdn_nomis_db_1" {
  value = "${module.nomis_db_1.public_fqdn}"
}

output "internal_fqdn_nomis_db_1" {
  value = "${module.nomis_db_1.internal_fqdn}"
}

output "private_ip_nomis_db_1" {
  value = "${module.nomis_db_1.private_ip}"
}

output "db_disks_nomis_db_1" {
  value = "${module.nomis_db_1.db_size_parameters}"
}

# map (tidier)
output "nomis_db_1" {
  value = {
    ami_id        = "${module.nomis_db_1.ami_id}",
    public_fqdn   = "${module.nomis_db_1.public_fqdn}",
    internal_fqdn = "${module.nomis_db_1.internal_fqdn}",
    private_ip    = "${module.nomis_db_1.private_ip}",
    db_disks      = "${module.nomis_db_1.db_size_parameters}",
    nomis_db_1   = "ssh ${module.nomis_db_1.public_fqdn}",
  }
}
